...
def drop_nan_features(
    ...
    1. Replace NaN/inf/-inf with null (existing nulls remain missing)
    2. If a feature has >cell_threshold missing rows → drop the feature
    3. Drop remaining rows that have any null in feature columns

    Returns a LazyFrame with no NaN/Inf/null in feature columns.
    """
    cols = lf.collect_schema().names()
    feat_cols = _find_feat_cols(cols)
    meta_cols = _find_meta_cols(cols)

    # Polars distinguishes IEEE NaN from null; unify all missing feature values.
    lf = lf.with_columns(
        [
    ...
    # Collect missing-value counts per feature to decide which to drop
    nan_counts = lf.select([pl.col(c).is_null().sum().alias(c) for c in feat_cols]).collect().row(0, named=True)
...
def clip_outliers(
    ...
    n_clipped = (
        lf.select([(pl.col(c).abs() > threshold).sum().alias(c) for c in feat_cols]).collect().row(0, named=True)
    )
    ...
    logger.info(
        "Outlier clipping: capped %d values across %d features at ±%.0f (all %d features retained)",
        total_clipped,
        n_affected_feats,
        threshold,
        len(feat_cols),
    ...
    lf = lf.select([pl.col(c) for c in meta_cols] + [pl.col(c).clip(-threshold, threshold) for c in feat_cols])
    return lf
